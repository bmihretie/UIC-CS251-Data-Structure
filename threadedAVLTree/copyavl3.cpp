 ...
 void _insertHelper(KeyT key, ValueT& value)
  {
   ...
   Size++;
    ...
    while (!nodesStack.empty())
    {
      ...
      if (cur->Height == hCur) // since the height is not changed we don't need to update it
		  break;
  ...
  void _inorder(ostream& output, NODE* cur) const
  {
     ...
     { 
	     ...
	     //if the node is threaded and THREAD denotes the next inorder key, output  (key,value,height) 
...
//       // first and insert that (key, value) pair first.  That way
//       // all the nodes to the left will properly go to the left,
//       // all the node to the right lue);
// 	_copytree(cur->Left);
		
// 	  //check on the right subtree if it is threaded or not 
// 		if(cur->isThreaded)will properly go to the right.
//       // In this case we also check if cur is threaded or not 
//       // 
// 	//insert(cur->Key, cur->Value);
// 	 _insertHelper(cur->Key, cur->Va
// 			 cur = nullptr;
// 		else
// 			_copytree(cur->Right);
...
//      }
  ...
  void _RightRotate(NODE* Parent, NODE* N)
  {
	...
	// rotate the tree
	 ...
	 //cout << "L key" <<  L->Key << endl;
     //cout << "L->Right expectin18 " <<  L->Right->Key << endl;
	 if(B == nullptr)
		 N->Left = NULL;
...
// 	int rR = (R->Right== NULL || R->isThreaded) ? -1 : R->Right->Height;
	R->Height = 1 + std::max(rR,N->Height);
